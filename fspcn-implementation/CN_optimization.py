...
class GA:
    ...
    def genetic_algorithm(self, graph, num_communities, population_size=50, generations=100, mutation_rate=0.1):
        """
        ...
        def fitness_function(chromosome):
            ...
            resource_usage = sum(
                graph.nodes[node].get('RAM', 0) * self.w_RAM +
                graph.nodes[node].get('STO', 0) * self.w_TB +
                graph.nodes[node].get('IPT', 0) * self.w_IPT
    ...
    def calculate_fitness(self, graph, communities, num_communities):
        """
        Menghitung fitness dari komunitas yang diberikan.
        :param graph: Graf NetworkX.
        :param communities: List komunitas untuk setiap node.
        :param num_communities: Jumlah komunitas.
        ...
        for node, community_id in enumerate(communities):
            ...
            resource_usage += (
                node_data.get('RAM', 0) * self.w_RAM +
                node_data.get('STO', 0) * self.w_TB +
                node_data.get('IPT', 0) * self.w_IPT
            )
        ...
        fitness = (
            self.omega_1 * resource_usage +
            self.omega_2 * sum((count - (len(communities) / num_communities))**2 for count in node_distribution.values()) +
            self.omega_3 * sum(node_usage.values())
        )
        return -fitness  # Minimalkan fitness
