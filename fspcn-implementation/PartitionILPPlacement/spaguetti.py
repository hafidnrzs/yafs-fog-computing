...
if myConfiguration_ == "tinny":
    ...
    func_NETWORKGENERATION = (
        "nx.barbell_graph(5, 1)"  # algorithm for the generation of the network topology
    )
...
def writeStatisticsAllocationILP(clientId, servId, devId):
    if not devId == cloudId:
        ...
        dist_ = nx.shortest_path_length(
            G, source=clientId, target=devId, weight="weight"
        )

        mykey_ = dist_
        if mykey_ in statisticsDistancesRequestILP:
            statisticsDistancesRequestILP[mykey_] = (
        ...
        if mykey_ in statisticsDistanceDeadlineILP:
            statisticsDistanceDeadlineILP[mykey_] = (
        ...
        if mykey_ in statisticsServiceInstancesILP:
            statisticsServiceInstancesILP[mykey_] = (
...
def writeStatisticsDevicesILP(servId, devId):
    ...
    if not devId == cloudId:
        ...
        if mykey_ in nodeBussyResourcesILP:
            nodeBussyResourcesILP[mykey_] = nodeBussyResourcesILP[mykey_] + float(
                myServicesResources[servId]
            )
        else:
...
def normalizeStatisticsDevicesILP():
    for i in nodeBussyResourcesILP.items():
        devId = i[0]
        mypercentageResources_ = float(nodeBussyResourcesILP[devId]) / float(
            nodeResources[devId]
        )
        ...
        if mykey_ in statisticsCentralityResourcesILP:
            statisticsCentralityResourcesILP[mykey_] = (
...
def writeStatisticsAllocation(tempServiceAlloc, clientId, appId):
    ...
    for talloc_ in tempServiceAlloc.items():
        dist_ = nx.shortest_path_length(
            G, source=clientId, target=talloc_[1], weight="weight"
        )
...
def writeStatisticsDevices(service2DevicePlacementMatrix):
    for devId in G.nodes:
        mypercentageResources_ = float(nodeBussyResources[devId]) / float(
            nodeResources[devId]
        )
        mycentralityValues_ = centralityValuesNoOrdered[devId]
        mykey_ = (mycentralityValues_, mypercentageResources_)
        if mykey_ in statisticsCentralityResources:
            statisticsCentralityResources[mykey_] = (
...
def weightNetwork(appdId):
    ...
    for e in G.edges:
        G[e[0]][e[1]]["weight"] = float(G[e[0]][e[1]]["PR"]) + (
            size / float(G[e[0]][e[1]]["BW"])
        )
...
def placeSubAppInCommunity(appId, subAppCommunity, orderedDevices):
    ...
    for devId in orderedDevices:
        availableResourcesNodes[devId] = (
            nodeResources[devId] - nodeBussyResources[devId]
        )
    ...
    for devId in orderedDevices:
        ...
        if availableResourcesNodes[devId] < neededResources:
            if verbose_log:
                print(
                    "            Performed allocation of service set "
                    + str(subAppCommunity)
                    + " of app "
                    + str(appId)
                    + " in device "
            ...
            if verbose_log:
                print(
                    "            Not allocatted due to not enough resources in device"
                )
    ...
    for appCommu in appsCommunities[appId]:
        if (
            ...
            if allocat_:
                ...
                if verbose_log:
                    print(
                        "        Allocated. Remaining services " + str(servicesToPlace)
                    )
            ...
            for serviceSet in listOfTransitiveClosures:
                if (
                    ...
                    for service in serviceSet:
                        requiredResources = (
                            requiredResources + appsResources[appId][service]
                        )
        ...
        for serviceSet in listOfTransitiveClosures:
            ...
            if verbose_log:
                print(
                    ...
                    + str(listOfTransitiveClosures)
                )
            if verbose_log:
                print(
                    "        and the allocation is defined as " + str(tempServiceAlloc)
                )
            return True, tempServiceAlloc
...
initialIndx = int(
    (1 - PERCENTATGEOFGATEWAYS) * len(G.nodes)
...
for i in range(0, TOTALNUMBEROFAPPS):
    ...
    mapping = dict(
        zip(APP.nodes(), range(numberOfServices, numberOfServices + len(APP.nodes)))
    )
...
service2DevicePlacementMatrix = [
    [0 for j in range(len(G.nodes))] for i in range(numberOfServices)
]
        ...
        for myCommunity in sortedCommunities:
            if clientId in myCommunity[0]:
                if appId in community2AppPlacementDict[myCommunity[0]]:
                    ...
                    placed_, servicePlacement = placeAppInCommunity(
                    ...
                    if placed_:
                        ...
                        for servId, deviceId in servicePlacement.items():
                            ...
                            nodeBussyResources[deviceId] = (
                                nodeBussyResources[deviceId]
                                + appsResources[appId][servId]
                            )
...
print(
    "Number of services in cloud (partition) (servicesInCloud): " + str(servicesInCloud)
)
...
if ILPoptimization:
    ...
    for gtwId in allTheGtws:
        for devId in G.nodes:
            networkdistances[(gtwId, devId)] = nx.shortest_path_length(
                G, source=gtwId, target=devId, weight="weight"
            )
    ...
    UserServiceDevAssignment = {
        ...
        for comb in assignCombinations
    }
    ...
    for usrservId in userServices:
        problem += (
            pulp.lpSum(
                [(UserServiceDevAssignment[(usrservId, devId)]) for devId in fognodes]
            )
            == 1,
            ...
            pulp.lpSum(
                [
                    (
                        ...
                        * myServicesResources[usrservId[1]]
                    )
                    for usrservId in userServices
                ]
            )
    ...
    print(
        "Number of ILP variables (assignCombinations): " + str(len(assignCombinations))
    )
    print("************")
    ...
    print(
        "Number of services in cloud (ILP) (servicesInCloud): " + str(servicesInCloud)
    )
...
if generatePlots:
    ...
    plt.scatter(
        x,
        y,
        s=list(map(lambda x: x * 100, area)),
        c="blue",
        alpha=0.5,
        label="partition",
    )
    plt.scatter(
        xILP,
        yILP,
        s=list(map(lambda x: x * 100, areaILP)),
        c="red",
        alpha=0.5,
        label="ilp",
        marker="X",
    ...
    lgnd = plt.legend(
        fontsize=20, loc="upper left", bbox_to_anchor=(0.15, 1.12), ncol=2
    )
    ...
    for i in statisticsServiceInstances.items():
        ...
        if mykey_ in poststatisticsServiceInstances:
            poststatisticsServiceInstances[mykey_] = (
    ...
    for i in statisticsServiceInstancesILP.items():
        ...
        if mykey_ in poststatisticsServiceInstancesILP:
            poststatisticsServiceInstancesILP[mykey_] = (
