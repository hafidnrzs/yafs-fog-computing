...
class Sim:
    ...
    **Main variables to coordinate with algorithm:**
        ...
        while not self.stop:
            ...
            if (
                ...
                pipe_id = "%s%s%i" % (
                    ...
                    message.dst,
                    message.idDES,
                )  # app_name + module_name (dst) + idDES
                ...
                try:
                    ...
                    self.last_busy_time[link] = last_used
                    self.env.process(
                        self.__wait_message(message, latency_msg_link, shift_time)
                    )
                except:
    ...
    def __update_node_metrics(self, app, module, message, des, type):
        try:
            ...
            self.logger.critical(
                "Make sure that this node has been removed or it has all mandatory attributes - Node: DES:%i"
                % des
            )
            return 0
                    ...
                    if msg.name == register["message_in"].name:
                        ...
                        if not register["message_out"]:
                            ...
                            if register[
                                ...
                                **register["param"]
                            ):  ### THRESHOLD DISTRIBUTION to Accept the message from source
                                if not register["module_dest"]:
                                    # it is not a broadcasting message
                                    self.logger.debug(
                                        "(App:%s#DES:%i#%s)\tModule - Transmit Message:\t%s"
                                        % (
                                            app_name,
                                            ides,
                                            module,
                                    ...
                                    self.logger.debug(
                                        "(App:%s#DES:%i#%s)\tModule - Broadcasting Message:\t%s"
                                        % (
                                            app_name,
                                            ides,
                                            module,
    ...
    def get_alloc_entities(self):
        ...
        for id_des_process in self.alloc_source:
            ...
            alloc_entities[src_deployed["id"]].append(
                src_deployed["app"] + "#" + src_deployed["module"]
            )
    ...
    def print_debug_assignaments(self):
            ...
            for module in self.alloc_module[app]:
                ...
                for des in deployed:
                    fullAssignation[des] = {
                        ...
                        "Module": module,
                    }  # DES process are unique for each module/element
        ...
        for k in self.alloc_DES:
            print(
                ...
                self.alloc_DES[k],
                "\t|",
                self.alloc_source[k]["name"] if k in self.alloc_source.keys() else "--",
                "\t\t|",
    ...
    def __add_mobile_agent(self, ides, gme):
        ...
        while (
            (len(gme.path) - 1 > gme.current_position)
            and not self.stop
            and self.des_process_running[ides]
        ):
    ...
    #     animation.make_video(
    #         output_file=pathFile, framerate=10, linewidth=1.0, G=self.topology.G
    #     )
    ...
    def run(
        ...
        for place in self.placement_policy.values():
            for app_name in place["apps"]:
                ...
                place["placement_policy"].initial_allocation(
                    self, app_name
        ...
        *Simpy.run.until* wait to all pipers are empty. So, hundreds of messages should be service... We force with the stop
        ...
        distribution = deterministic_distribution(
            name="SIM_Deterministic", time=time_shift
        )
        self.env.process(
