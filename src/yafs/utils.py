...
def toMeters(geometry):
    project = partial(
        pyproj.transform, pyproj.Proj(init="EPSG:4326"), pyproj.Proj(init="EPSG:32633")
    )
...
def draw_topology(topology, alloc_entity):
    ...
    nx.draw_networkx_nodes(
        G, nodelist=G.nodes() - nodesM, node_shape="s", pos=pos, ax=ax
    )
    nx.draw_networkx_nodes(
        G,
        nodelist=nodesM,
    ...
    nx.draw_networkx_edges(
        G, edgelist=edgesM, pos=pos, style="dashed", width=0.8, ax=ax
    )
...
def haversine_distance(origin, destination):
    ...
    a = math.sin(dlat / 2.0) * math.sin(dlat / 2.0) + math.cos(
        math.radians(origin[0])
    ) * math.cos(math.radians(destination[0])) * math.sin(dlon / 2.0) * math.sin(
        dlon / 2.0
    )
