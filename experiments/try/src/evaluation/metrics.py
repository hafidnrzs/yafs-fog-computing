...
# --- FUNGSI PERHITUNGAN DELAY (ADRA & ADSA) ---
# Logika ini sekarang berada di dalam file metrics.py

def _calculate_path_delay(path: list, graph: nx.Graph, packet_size: float) -> float:
    """Menghitung total delay di sepanjang sebuah path berdasarkan Eq. 31."""
    total_delay = 0
...
def calculate_adsa(target_app: dict, placement_matrix: dict, network_graph: nx.Graph, packet_size: float) -> float:
    ...
    for service_name in service_list:
        # Mengambil lokasi dari placement_matrix
...
# --- FUNGSI METRIK EVALUASI LAINNYA ---

def calculate_placement_metrics(placement_matrix, network, applications, config):
    """
    Menghitung metrik terkait penempatan seperti jumlah penempatan di cloud.
    """
    total_placements = sum(len(nodes) for nodes in placement_matrix.values())
    if total_placements == 0:
        return {'cloud_placement_rate': 0.0}

    cloud_node_id = config.CLOUD_NODE_ID
    placements_in_cloud = sum(1 for nodes in placement_matrix.values() for node_id in nodes if node_id == cloud_node_id)
    
    cloud_placement_rate = placements_in_cloud / total_placements
    
    return {'cloud_placement_rate': cloud_placement_rate}


def calculate_availability_metric(placement_matrix, network, applications, users, config):
    """
    Menghitung metrik ketersediaan (simplified version).
    """
    if not users:
        return 0.0

    app_service_map = {app['name']: [m['name'] for m in app['module']] for app in applications}
    ...
    for user_req in users:
        ...
        for service in services_for_app:
            if service not in placement_matrix:
                all_paths_exist = False
                break
            
            # Cek apakah ada setidaknya satu jalur ke salah satu replika layanan
            has_path_to_service = any(nx.has_path(network, requester_gateway, node_id) for node_id in placement_matrix[service])
            if not has_path_to_service:
                all_paths_exist = False
                break
    ...
    return available_requests / total_requests if total_requests > 0 else 0.0


def calculate_delay_metrics(placement_matrix, network, applications, users, config):
    """
    Menghitung metrik delay rata-rata (ADRA dan ADSA) untuk semua aplikasi.
    """
    delay_results = []
    ...
    for app_id, app_data in app_map.items():
        ...
        if user_for_app:
            ...
            entry_module_name = next((t.get("module") for t in app_data.get("transmission", []) if t.get("message_in") == f"M.USER.APP.{app_id}"), None)
            
            if entry_module_name and entry_module_name in placement_matrix:
                # Ambil lokasi terdekat dari entry module ke requester
                entry_nodes = placement_matrix[entry_module_name]
                min_adra = float('inf')
                for node in entry_nodes:
                    current_adra = calculate_adra(requester_node, node, network, packet_size)
                    if current_adra < min_adra:
                        min_adra = current_adra
                adra = min_adra

        total_delay = adsa + adra if adra != float('inf') else float('inf')
        delay_results.append({'app_id': app_id, 'adra': adra, 'adsa': adsa, 'total_delay': total_delay})

    return delay_results


def print_evaluation_summary(results):
    """Mencetak ringkasan hasil evaluasi."""
    print("\n" + "="*50)
    print("--- HASIL EVALUASI EKSPERIMEN FSPCN ---")
    print("="*50)

    cloud_rate = results.get('placement_metrics', {}).get('cloud_placement_rate', 0)
    print("\n[Metrik Penempatan]")
    print(f"  - Tingkat Penempatan di Cloud: {cloud_rate:.2%}")

    availability = results.get('availability', 0)
    print("\n[Metrik Ketersediaan]")
    print(f"  - Rata-rata Ketersediaan Aplikasi: {availability:.2%}")

    delay_df = pd.DataFrame(results.get('delay_metrics', []))
    if not delay_df.empty:
        # Ganti inf dengan NaN agar bisa diabaikan saat menghitung mean
        delay_df.replace(float('inf'), pd.NA, inplace=True)
        avg_total_delay = delay_df['total_delay'].mean()
        print("\n[Metrik Delay]")
        print(f"  - Rata-rata Total Delay (ADRA + ADSA): {avg_total_delay:.2f} ms")
        print("\nDetail Delay per Aplikasi:")
        print(delay_df.to_string(index=False, float_format="%.2f"))

    print("\n" + "="*50)
    print("Catatan: Metrik 'Meet Deadline' dan 'Resource Usage' memerlukan simulasi dinamis (YAFS) untuk dihitung.")
    print("="*50)
